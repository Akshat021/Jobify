...
const getAllJobs = async (req, res) => {
  ...
  const limit = Number(req.query.limit) || 10; // not sending from front-end
...
const updateJob = async (req, res) => {
  ...
  const updatedJob = await Job.findByIdAndUpdate({ _id: jobId }, req.body, {
    // this approach does not trigger the middleware(hooks) in jobs model
    new: true,
    runValidators: true,
  });
...
const showStats = async (req, res) => {
  ...
  stats = stats.reduce((acc, curr) => {
    // return stats as an object rather than like an Array.
